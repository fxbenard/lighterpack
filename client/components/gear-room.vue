...
<template>
    <div class="lpGearRoom">
        ...
        <!-- Compare panel -->
        <div :class="['lpGearRoomComparePanel', { 'lpGearRoomComparePanel--open': compareOpen }]">
            <div class="lpGearRoomCompareHeader">
                <span class="lpGearRoomCompareTitle">Compare {{ compareItems.length }} items</span>
                <button class="lpGearRoomCompareClose" @click="compareOpen = false">✕ Close</button>
            </div>
            <div class="lpGearRoomCompareScroll">
                <table class="lpGearRoomCompareTable">
                    <colgroup>
                        <col class="lpGearRoomCompareColAttr">
                        <col v-for="item in compareItems" :key="item.id" class="lpGearRoomCompareColItem">
                    </colgroup>
                    <thead>
                        <tr>
                            <th class="lpGearRoomCompareAttrCell lpGearRoomCompareAttrHeader"></th>
                            <th v-for="item in compareItems" :key="item.id" class="lpGearRoomCompareItemHeader">
                                <span class="lpGearRoomCompareItemName">{{ itemDisplayName(item) }}</span>
                                <button class="lpGearRoomCompareRemove" @click="selected = selected.filter(id => id !== item.id)">✕</button>
                            </th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td v-for="item in compareItems" :key="item.id" class="lpGearRoomCompareValueCell lpGearRoomCompareNameCell">
                        ...
                        <tr>
                            ...
                            <td v-for="item in compareItems" :key="item.id" class="lpGearRoomCompareValueCell lpGearRoomCompareDescCell">
                                {{ item.description || '—' }}
                            </td>
                        </tr>
                        <!-- Category row -->
                        ...
                        <tr>
                            ...
                            <td v-for="item in compareItems" :key="item.id" class="lpGearRoomCompareValueCell">
                                {{ itemUsedInLists(item.id) }} list{{ itemUsedInLists(item.id) !== 1 ? 's' : '' }}
                            </td>
                        </tr>
                    </tbody>
                </table>
    ...
    computed: {
        ...
        compareItems() {
            return this.selected
                .map(id => this.library.getItemById(id))
                .filter(Boolean);
        },
        compareMinWeight() {
            const weights = this.compareItems.map(i => i.weight || 0).filter(w => w > 0);
